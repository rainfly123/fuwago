...
func queryStrV2Handler(w http.ResponseWriter, req *http.Request) {
	geohash := req.FormValue("geohash")
	radius := req.FormValue("radius")
	biggest := req.FormValue("biggest")
	if len(geohash) < 5 || len(biggest) < 1 || len(radius) < 2 {
	...
	jsonres := JsonResponseData{JsonResponse{0, "OK"}, QueryStrV2(longitude, latitude, uint32(radiusint), big)}
	b, _ := json.Marshal(jsonres)
	io.WriteString(w, string(b))
	return
}
